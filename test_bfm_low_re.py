"""Headless smoke test for BFM at Re=426248 (the crash case from the GUI)."""
import sys, os, threading, warnings
sys.path.insert(0, os.path.dirname(__file__))
if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")
if hasattr(sys.stderr, "reconfigure"):
    sys.stderr.reconfigure(encoding="utf-8", errors="replace")

warnings.filterwarnings("always", category=RuntimeWarning)

from solver.core_bfm import run_bfm_simulation

class MockRoot:
    @staticmethod
    def after(delay, fn, *args): fn(*args)

class MockGUI:
    root = MockRoot()
    kill_event = threading.Event()
    pause_event = threading.Event()
    solver_running = True
    def update_live_metrics(self, i, res, elapsed): pass
    def update_live_results(self, matrices, cl, cd, ld, *args): pass
    def set_body_mask(self, mask): pass
    def append_history_row(self, i, res, cl, cd, ld): pass
    def display_final_results(self, matrices, cl, cd, ld, *args, **kwargs):
        print(f"  Final: Cl={cl:.4f}  Cd={cd:.4f}  L/D={ld:.4f}")

p = {
    "airfoil":       "2412",
    "n_xi":          512,
    "n_eta":         128,
    "ogrid_R":       15.0,
    "alpha_stretch": 8.0,
    "re":            426248.0,
    "aoa":           0.0,
    "dt":            2e-4,
    "max_iters":     20000,
    "hist_interval": 50,
    "t_wall":        320.0,
    "t_inf":         300.0,
}

print("=== BFM low-Re smoke test ===")
print(f"Re={p['re']:.3g}  dt={p['dt']:.2e}  grid={p['n_xi']}x{p['n_eta']}  max_iters={p['max_iters']}")
print()

try:
    run_bfm_simulation(MockGUI(), p)
    print()
    print("=== PASSED: solver completed without NaN/overflow ===")
except Exception as e:
    print()
    print(f"=== FAILED: {type(e).__name__}: {e} ===")
    sys.exit(1)
